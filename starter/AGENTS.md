...
- **Say which MODEL you are, not just which agent.** Prefix the command with
  `HX_MODEL="<your model>"` (e.g. `HX_MODEL="GPT-5.6-sol"`, `HX_MODEL="Opus 5"`), or
  fill the `MODEL:` line in the stub and run `bash handoff/hx.sh reindex`. "Codex
  said X" is not enough six weeks later — gpt-5.6-sol and gpt-5.6-terra are
  different witnesses.
