...
- **BUGS.md** — the recurring-mistake registry. ⚠ **READ IT BEFORE WRITING OR EDITING ANY
  ...
  It is a separate file on purpose — this one is loaded in full every session and has a size
  budget; a trap list does not.
...
## Handover to the other agent (ONLY if a second agent — e.g. Claude Code — also works here)
...
- **Identities carry the model** (`codex (ChatGPT Sol 5.6)`, `claude (Opus 5)`) —
  `hx.sh` fills it in from the two variables at its top, so never hand-type one.
  Keep them current: "Codex said X" is not enough six weeks later, because two
  releases of the same CLI are different witnesses.
...
<!-- The traps that produce a confident WRONG ANSWER rather than an error. Keeping them in
     ...
     in full every session, under a size budget — from growing without limit. -->
