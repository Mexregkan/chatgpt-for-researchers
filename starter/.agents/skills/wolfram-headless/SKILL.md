...
description: Run heavy headless wolframscript jobs reliably: diagnose the misleading 'license error' (usually a memory crash, not licensing), avoid the literal-Greek .wls silent-corruption bug, and treat kernel errors as stop-and-fix.
---

# wolfram-headless — reliable headless Wolfram under a coding agent

Hard-won rules for running `wolframscript` from a coding agent's shell. Two findings dominate:
...
at the end and poll the file, rather than relying on the returned stdout. Run long jobs detached (`nohup wolframscript -file job.wls > /tmp/job.log 2>&1 &`)
and poll the log/result file, grepping for `license error|<your done marker>|ABORT`.
...
- [ ] Run detached (`nohup … &`) + poll the log for `license error` and a done-marker.
...
## See also
- `hooks/wolfram-license-notice.sh` (companion hook, in this skill's directory) —
  auto-flags the misleading "license error" via a PostToolUse hook so the next
  person doesn't misdiagnose it as a licensing problem. Opt-in — see the install
