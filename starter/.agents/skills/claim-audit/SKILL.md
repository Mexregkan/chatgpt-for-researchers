...
```bash
bash .agents/skills/claim-audit/gate_audit.sh <script>
```
...
2. **What that licenses** — the headline, already narrowed by steps 1–2, carrying one of the
   statuses from `AGENTS.md § Research-claim discipline`.
...
- Editing `gate_audit.sh` without re-running `bash .agents/skills/claim-audit/selftest.sh`.
...
`AGENTS.md § Research-claim discipline` (the status vocabulary this skill's step 3 uses).
