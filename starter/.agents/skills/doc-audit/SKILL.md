---
name: doc-audit
description: Evaluate one project document in detail — a workbook section, big picture, strategy map, handoff message, paper, brief, or primer — without editing it. Runs the mechanical lint, builds a claim ledger, types every headline against the eight claim statuses in AGENTS.md, traces cited evidence to the scripts and logs that exist, detects drift against the strategy map / CHANGELOG / newest handoff, sends the load-bearing claims to a fresh-context reader, and writes a report under audits/. Use when asked to audit, evaluate, review, check, or proofread a .tex or .md document. Not a referee report — no recommendation, no author, no edits.
---
...
appendix, `AGENTS.md`. The rubric for each type is in [`rubrics.md`](rubrics.md); the
...
```bash
bash .agents/skills/doc-audit/doc_lint.sh <file>        # LINTMAX=80 to list more lines
```
...
```bash
bash .agents/skills/doc-audit/selftest.sh                 # must print 0 failed
```
...
6. definitions and conventions used (the ones pinned in `AGENTS.md § Conventions`);
...
`AGENTS.md § Research-claim discipline` — definition, exact identity, proved theorem,
...
- **DRIFT** — the same statement in `strategy-map.md`, `CHANGELOG.md`, `bigPicture.tex`,
  `brief.tex`, `AGENTS.md § Current status`, the newest handoff, `BUGS.md`: same strength,
...
- **CONVENTION** — against `AGENTS.md § Conventions`. Any quantity transported from another
...
- Treating the strategy map, `CHANGELOG.md`, a handoff or `AGENTS.md` as ground truth for
...
Agents: `.codex/agents/doc-sweeper.toml`, `.codex/agents/doc-auditor.toml`.
