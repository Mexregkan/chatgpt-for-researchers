...
- **C1 Typed headlines.** Every headline is one of the eight statuses in
  `AGENTS.md § Research-claim discipline`, and the type as written is the type the evidence
  supports. ★ (`BUGS.md` § I)
...
- **C5 Conventions.** Every convention the document uses is the one pinned in
  `AGENTS.md § Conventions`, and a quantity transported from another source's convention
...
- **R5 Consistency with the standing rules.** Anything ruled out here is also ruled out in
  `AGENTS.md` and `next-session-prompts.md`.
...
## `AGENTS.md` / `AGENTS.md`, `README.md`, task queues

- **A1** Currency: `§ Current status` and `next-session-prompts.md` agree with the strategy
  map.
- **A2** Size: within the budget the file's own header states (a AGENTS.md that grows past
