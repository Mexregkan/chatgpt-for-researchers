---
name: apply-pipeline
description: The write-side inverse of check-pipeline: apply an optimization or correction the Pipeline/ doc (backed by the authoritative notes) describes to the CODE, with diff preview, ground-truth protection, and a control re-run.
---
...
| `pipeline-auditor` (sub-agent) | code via doc → bug/opt report | nothing (read-only) |
...
- **HIGH** (PREVIEW a diff + get explicit approval FIRST): anything touching the core math, a
  ...
  those are the user's ground truth (see `AGENTS.md` "AI-generated outputs"); regenerating data
...
- If the target notebook is **open in a live-kernel editor** (e.g. a `.wb` in VS Code via the
  Wolfbook MCP): edit it **through that editor's tools**, NOT by overwriting the file on disk — a blind
  disk write won't reload into the open editor and you'll fight a stale buffer. If it's
  closed, editing the file directly is fine.
