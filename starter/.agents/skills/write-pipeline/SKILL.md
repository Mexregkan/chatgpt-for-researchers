---
name: write-pipeline
description: Write (or refresh) a Pipeline/ doc mapping a big code's data flow, key symbols, and gotchas, so sessions read the map before the (large) source. Use when a load-bearing code has grown too big to hold in context.
---
...
## Gotchas / guardrails
Bullet the non-obvious traps, each pointing at the authority (a workbook label, an AGENTS.md convention, a
...
- Cite `workbook.tex` labels / paper equations and `AGENTS.md` conventions rather than
...
- Mention the new file to the user; suggest running `$check-pipeline <file>` to confirm it matches,
  and a `pipeline-auditor` sub-agent pass to hunt for bugs/optimizations before you rely on it.
