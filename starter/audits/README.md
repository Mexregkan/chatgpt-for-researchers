...
paper, a brief, a primer or `AGENTS.md`. Not referee reports: no recommendation, no author,
...
- **Shape:** `.agents/skills/doc-audit/report-template.md` — summary verdict, ranked
...
- **Tooling:** `bash .agents/skills/doc-audit/doc_lint.sh <file>` is the mechanical
  pre-filter; `bash .agents/skills/doc-audit/selftest.sh` is its regression suite. The
  read-only agents `doc-sweeper` and `doc-auditor` live in `.codex/agents/`.
