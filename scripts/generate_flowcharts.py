...
def chart_setup():
    ...
    nd(g, 'boot',   'Run scripts/bootstrap.sh (terminal)\n'
                    ...
                    'installs the core + only the matching prompts', S_PROC)
    nd(g, 'open',   'Open Codex (VS Code or terminal)\n'
                    'describe your project in the first message',  S_PROC)
    nd(g, 'fill',   'Paste the prompt\n'
                    'Codex writes AGENTS.md + the trinity\n'
...
def chart_session():
    ...
    nd(g, 'start',  'Open Codex',                            S_TERM)
    nd(g, 'load',   'AGENTS.md auto-loads\n'
    ...
    nd(g, 'upd',    'Update workbook.tex\n'
                    'sync brief.tex\n'
                    'update AGENTS.md status',                     S_PROC)
    nd(g, 'ddon',   'Session\ndone?',                              S_DEC)
    nd(g, 'wrap',   'Update AGENTS.md: last result + next step\n'
