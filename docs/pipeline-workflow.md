...
a 400 KB Mathematica notebook, a numerics engine, a solver — that Codex (and you) can no
longer hold in context or read top-to-bottom. This workflow gives every such code a short,
living "pipeline" document that Codex reads *first*, keeps in lockstep with the code, and
...
until invoked — a skill costs nothing until you mention `$write-pipeline`, an agent nothing
until you spawn it, and the guard hook stays silent until a `Pipeline/` doc exists). Then adopt the
...
A big code file is a wall. Codex opening `giant-notebook.nb` (300+ cells) burns context and
...
in which cell, the non-obvious traps, and what it consumes and produces. Codex reads the map
...
| `pipeline-guard` | hook | on every edit | PostToolUse nudge: after you edit a main code, it tells Codex to run `$check-pipeline` (+ an auditor pass); after you edit a pipeline doc, it tells Codex to run `$apply-pipeline`. Nudge-only, never blocks; quiet on scratch files. |
...
- **The auditor is an agent, the rest are skills.** An audit is an open-ended search — ideal
  ...
  session has, so `$apply-pipeline` is a skill, not an agent. In Codex the auditor's
  read-only property is not just a promise: its agent definition sets
  `sandbox_mode = "read-only"`, so the platform enforces it.
...
2. **Add the guard hook** so edits nudge the right tool. It is a short PostToolUse shell script
   matched on the file-editing tool (`apply_patch`, aliases `Edit`/`Write`) that reads the edited
   path and prints a one-line reminder (run `$check-pipeline` on a code edit, `$apply-pipeline`
   on a doc edit). It only emits `additionalContext`; it never blocks. Keep it scoped to your
   *main* codes + `Pipeline/` so it stays quiet on scratch scripts. Register it in
   `.codex/hooks.json` and trust it via `/hooks` — Codex refuses to run un-reviewed hooks.
...
4. **Write the workflow into your `AGENTS.md`** as a standing rule (the three invariants + the
   lifecycle + "read `Pipeline/README.md` before opening any large code"). The hook enforces the
   mechanics; the `AGENTS.md` section makes every session *follow the intent*, including for the
...
5. **Register the sub-agent** (a TOML file under `.codex/agents/` with `name`, `description`,
   `developer_instructions`, and `sandbox_mode = "read-only"`). Define it once and spawn it by
   asking Codex ("spawn the pipeline-auditor agent on Pipeline/main.md"); if a fresh definition
   is not picked up mid-session, start a new session.

The skills and agent are straightforward to port: each is a `SKILL.md` (or agent `.toml`) whose
...
- **A hook cannot see MCP/live-kernel edits.** File-edit hooks fire on `apply_patch`, not on a
  notebook edited through a live-kernel MCP. Rely on the written `AGENTS.md` rule for those, not
  the hook.
- **A newly-defined sub-agent may not be live mid-session.** If Codex does not see it yet,
  start a new session; in the meantime, run the same instructions via a default sub-agent.
